...
async def _openai_variants(topic: str, tone: str, count: int, model: str) -> List[Dict[str, Any]]:
    if not OPENAI_API_KEY:
        return generate_variants(topic, tone, count)
    try:
    ...
    for t in TEMPLATES:
        if t["id"] == tpl_id:
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Missing variable: {var_name}")
...
# ----- app -----
fastapi_app = FastAPI(title="Smart Flow Systems — Social AI", version="0.7.0")
_scheduler_task: Optional[asyncio.Task] = None

# WSGI compatibility for gunicorn using a2wsgi
...
@fastapi_app.get("/plans")
async def plans():
    return {"currency": "GBP", "plans": ENTITLEMENTS}

@fastapi_app.post("/billing/checkout")
...
@fastapi_app.post("/generate", response_model=List[DraftOut])
async def generate(req: GenerateRequest):
    ...
    drafts = await _openai_variants(req.topic, req.tone, req.count, LLM_MODEL) if req.provider == "openai" else generate_variants(req.topic, req.tone, req.count)
    # onboarding nudge
...
@fastapi_app.post("/posts", response_model=PostOut)
...
@fastapi_app.get("/posts", response_model=List[PostOut])
...
@fastapi_app.post("/templates/render")
...
@fastapi_app.post("/admin/templates", dependencies=[])
...
@fastapi_app.post("/bulk/schedule_from_template")
...
@fastapi_app.get("/feedback")
async def list_feedback_api():
    rows = await store.list_feedback()
    return {"count": len(rows), "items": [asdict(r) for r in rows]}

@fastapi_app.post("/nps/submit")
async def nps_submit(req: NPSIn):
    rec = await store.add_nps(req.score, req.comment)
    return {"ok": True, "id": rec.id}

@fastapi_app.post("/referrals/create")
async def referrals_create():
    ref = await store.ensure_referral()
    return asdict(ref)

@fastapi_app.post("/referrals/redeem")
async def referrals_redeem(code: str):
    ok = await store.use_referral(code)
    return {"ok": ok}

@fastapi_app.get("/referrals/stats")
async def referrals_stats():
    prof = await store.get_profile()
    return {"referral": asdict(prof.referral) if prof.referral else None}

@fastapi_app.post("/leads/capture")
async def leads_capture(req: LeadIn):
    rec = await store.add_lead(req.name, req.email, req.message)
    return {"ok": True, "id": rec.id}

@fastapi_app.post("/events/log")
async def events_log(req: EventIn):
    if NOTIFY_WEBHOOK_URL:
        print(f"[WEBHOOK]: {NOTIFY_WEBHOOK_URL} -> {req.type}: {req.meta}")
    else:
        print(f"[EVENT]: {req.type}: {req.meta}")
    return {"ok": True}

@fastapi_app.post("/flags/set")
async def flags_set(key: str, enabled: bool = True):
    flags = await store.upsert_flag(key, enabled)
    return {"flags": flags}

@fastapi_app.get("/onboarding/status")
async def onboarding_status():
    prof = await store.get_profile()
    return {"onboarding": prof.onboarding_done}

# ----- engagement: smart replies -----
@fastapi_app.post("/engagement/replies")
async def engagement_replies(req: SmartReplyIn):
    replies = generate_replies(req.context, req.mood, req.count)
    return {"replies": replies}

# ----- analytics -----
@fastapi_app.get("/analytics/kpi")
async def analytics_kpi():
    ...
    return {
        ...
        "current_month": {"usage": usage, "limit": limit, "remaining": max(0, limit - usage)},
        "plan": prof.plan
    }

@fastapi_app.get("/analytics/posts_by_month")
async def analytics_posts_by_month():
    posts = await store.list_posts()
    ...
    for p in posts:
        try:
            ...
            if month not in by_month:
                by_month[month] = {"total": 0, "published": 0, "failed": 0, "scheduled": 0}
            by_month[month]["total"] += 1
            by_month[month][p.status] = by_month[month].get(p.status, 0) + 1
        except Exception:
            continue
    return {"by_month": by_month}

# ----- exports -----
@fastapi_app.get("/export/posts.csv")
async def export_posts_csv():
    posts = await store.list_posts()
    output = io.StringIO()
    writer = csv.DictWriter(output, fieldnames=["id", "platform", "content", "status", "scheduled_time", "external_id", "error", "created_at", "updated_at"])
    writer.writeheader()
    for p in posts:
...
@fastapi_app.get("/export/calendar.ics")
async def export_calendar_ics():
    posts = await store.list_posts(status="scheduled")
    ics_content = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Smart Flow Systems//Social AI//EN"]
    ...
    for p in posts:
        ...
        try:
            ...
            ics_content.extend([
                "BEGIN:VEVENT",
                f"UID:{p.id}@smartflow.systems",
                f"DTSTART:{dt_str}",
                f"DTEND:{dt_str}",
                f"SUMMARY:Post to {p.platform.upper()}",
                f"DESCRIPTION:{p.content[:100]}...",
                "END:VEVENT"
            ])
        except Exception:
            continue
...
# Note: WSGI compatibility already handled above